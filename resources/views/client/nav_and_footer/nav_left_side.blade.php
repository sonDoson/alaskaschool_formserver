<div style="width: 300px; height: 100%; border-right: 1px solid lightgrey;">
    <ul class="font-resize"style="margin-left: 30px; list-style:none; padding:0;">
        <li><a href="/cat/1" >{{ $category[1][$lang[0]] }}</a>
        <ul style="padding-left:30px">
            @foreach($category_item[1] as $key => $value)
            <li style="list-style:none; padding: 0;" ><a href="{{ '/cat/' . $value[key($value)]['id_category'] . '/' . $value[key($value)]['id']}}" >{{ $value[key($value)][$lang[0]] }}</a></li>
            @endforeach
        </ul>
        </li>
        <br />
